...
export const supabase = createClient(supabaseUrl, supabaseAnonKey, {
  auth: { persistSession: true, autoRefreshToken: true, detectSessionInUrl: false, storageKey: 'kingmenu.auth.token' },
});
...
function resolveJsonb(field: any, language: string, fallback = ''): string {
  ...
  if (typeof field === 'string') {
    try {
      ...
      if (typeof parsed === 'object' && parsed !== null) return parsed[language] || parsed['en'] || fallback;
    } catch { return field || fallback; }
  }
  if (typeof field === 'object') return field[language] || field['en'] || fallback;
  return fallback;
}
...
function resolveJsonbArray(field: any, language: string): string[] {
  if (!field) return [];
  let parsed = field;
  if (typeof field === 'string') { try { parsed = JSON.parse(field); } catch { return []; } }
...
export const getDishes = async (language: string = 'en') => {
  const { data, error } = await supabase
    .from('dishes')
    .select('*, dish_ingredients(quantity, unit, ingredient:ingredient_id(id, name, category, no_measure))')
...
export const getIngredients = async (language: string = 'en') => {
  ...
  return (data || []).map((ing: any) => ({
    ...ing,
    name: resolveJsonb(ing.name, language, 'Ingredient inconnu'),
...
export const updateUserPreferences = async (userId: string, preferences: any) => {
  const { data, error } = await supabase.from('user_preferences').upsert({ user_id: userId, ...preferences }, { onConflict: 'user_id' });
...
export const getSavedDishes = async (userId: string, language: string = 'en') => {
  const { data, error } = await supabase.from('saved_dishes').select('*, dishes(*)').eq('user_id', userId);
...
export const saveDish = async (userId: string, dishId: number | string) =>
  supabase.from('saved_dishes').insert({ user_id: userId, dish_id: Number(dishId) });

export const removeSavedDish = async (userId: string, dishId: number | string) =>
  supabase.from('saved_dishes').delete().eq('user_id', userId).eq('dish_id', Number(dishId));
